...
public class ItemAxe extends ItemTool {
    ...
    public ItemAxe(String id, String staticName, String texturePath, int maxAmount, String tier) {
    ...
    public void onPrimary(ItemStack itemStack, World world, EntityPlayer player, Vector2 mouseWorldPosition) {
        ...
        if (block != null && block.hasBlockTag(Tags.DESTROYABLE_AXE)) {
            ...
            if (soundIndex == 1) {
                SoundAsset axeSound = SoundHandler.getSound("sounds/items/Axe_Chop_2.wav");
                SoundHandler.playSound(axeSound, Game.getSettings().effectsVolume, false);
            }
            else {
                SoundAsset axeSound = SoundHandler.getSound("sounds/items/Axe_Chop_1.wav");
                SoundHandler.playSound(axeSound, Game.getSettings().effectsVolume, false);
            }
        }
        else {
            if (soundIndex == 1) {
                ...
                SoundAsset axeSound = SoundHandler.getSound("sounds/items/Tool_Thud_2.wav");
                SoundHandler.playSound(axeSound, Game.getSettings().effectsVolume, false);
            }
        }
    }
